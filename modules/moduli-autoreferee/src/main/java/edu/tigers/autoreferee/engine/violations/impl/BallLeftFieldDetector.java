...
public class BallLeftFieldDetector extends AViolationDetector
{
	...
	@Configurable(comment = "Area behind the goal that is still considered to be part of the goal for better goal detection")
	...
	private Optional<IRuleViolation> handleSideLineOff(final IVector2 ballPos,
	...
	protected Optional<IRuleViolation> handleGoalLineOff(final IVector2 intersection,
			final BotPosition lastTouched, final long ts)
	{
	...
	private IRuleViolation buildCornerKickResult(final BotID lastTouched, final IVector2 kickPos, final long ts)
	{
		FollowUpAction action = new FollowUpAction(EActionType.DIRECT_FREE, lastTouched.getTeamColor().opposite(),
				kickPos);
		RuleViolation violation = new RuleViolation(ERuleViolation.BALL_LEFT_FIELD, ts, lastTouched, action);
		return violation;
	}
	
	
	private IRuleViolation buildGoalKickResult(final BotID lastTouched, final IVector2 kickPos, final long ts)
	{
		FollowUpAction action = new FollowUpAction(EActionType.DIRECT_FREE, lastTouched.getTeamColor().opposite(),
				kickPos);
		RuleViolation violation = new RuleViolation(ERuleViolation.BALL_LEFT_FIELD, ts, lastTouched, action);
	...
	private IRuleViolation buildIcingResult(final BotID lastTouched, final IVector2 lastTouchedPos, final long ts)
	{
		...
		FollowUpAction action = new FollowUpAction(EActionType.INDIRECT_FREE, lastTouched.getTeamColor().opposite(),
				kickPos);
		RuleViolation violation = new RuleViolation(ERuleViolation.ICING, ts, lastTouched, action);
		return violation;
	}
	
	
	private IRuleViolation buildThrowInResult(final BotID lastTouched, final IVector2 kickPos, final long ts)
	{
