...
public class BotStopSpeedDetector extends APreparingViolationDetector
{
	...
	@Configurable(comment = "The number of milliseconds that a bot needs violate the stop speed limit to be reported")
	...
	public Optional<IRuleViolation> doUpdate(final IAutoRefFrame frame, final List<IRuleViolation> violations)
	{
		...
		if (optViolator.isPresent())
		{
			...
			SpeedViolation violation = new SpeedViolation(ERuleViolation.BOT_STOP_SPEED, frame.getTimestamp(), violator,
