...
public class LevenbergMarquardt
{
	...
	private static class FlyingBall implements LeastSquaresProblem
	{
		...
		public FlyingBall(final List<List<CamBall>> balls)
		{
			...
			for (List<CamBall> ballsOfFrame : balls)
			{
				for (CamBall ball : ballsOfFrame)
				{
					...
					observationsArray[counter][0] = ((double) TimeUnit.NANOSECONDS
							.toMillis(ball.getTimestamp() - firstTimestamp)) / 1000;
		...
		{
			return (ConvergenceChecker) new SimpleVectorValueChecker(-1, 0.1);
		}
		...
		public Evaluation evaluate(final RealVector point)
		{
			...
			for (double[] observation : observationsArray)
			{
				...
				double camera = observation[1];
				
				// evaluate expected position
				double[] pNow = evaluateModel(t, point);
				RealVector pNowVector = MatrixUtils.createRealVector(pNow);
				pNowVector = pNowVector.append(1);
				
				// field2image
				double[][] wInput = { { 5.8e+02, -8.8, -330, 1900000 },
						{ 28, -550, -270, 160000 },
						{ 0.1, -0.023, -0.99, 2500 } };
				RealMatrix W = MatrixUtils.createRealMatrix(wInput);
				RealVector x = W.operate(pNowVector);
				...
				residuals = residuals.append(real.subtract(y).getNorm());
